...
class Ui_MainWindow(object):
    ...
    # БЛЯЯЯЯЯЯТЬ Я В РОТ ЕБАААЛ ВАШЕ СРАНОЕ ДОБАЛЕНИЕ В БД НАХУЙ ОНО НАДО, ПРО ОБНОВЛЕНИЕ Я ВООБЩЕ МОЛЧУ
    # def add_song_in_playlist(self):
    #     song_name = self.Add_name_textbox.text()
    #     autor_song = self.Add_autor_textbox.text()
    #     albom = self.Add_albom_textbox.text()
    ...
    #     href_of_song = self.Add_href_textbox.text()
    #
    #     song_name = str(song_name)  # преобразование строки в переменную
    #     autor_song = str(autor_song)  # преобразование строки в переменную
    #     albom = str(albom)  # преобразование строки в переменную
    #     date_of_song = str(date_of_song)  # преобразование строки в переменную
    #
    #     # Подключение к БД
    #     connection = pymysql.connect(
    #         host=host,
    ...
    #     cursor = connection.cursor()
    #
    #     # Добавление данных в таблицу авторов
    #     # max_id_autor = "SELECT MAX(Autor_id) FROM autor"
    #     # cursor.execute(max_id_autor)
    #     # max_id_autor = cursor.fetchone()
    #     # max_id_autor = int(max_id_autor) + 1
    #     a = ("INSERT INTO autor (AutorName) VALUES (%s)", (autor_song,))
    #     cursor.execute(a)
    #
    #     # Добавление данных в таблицу музыки
    #     # max_id_music = "SELECT MAX(Music_id) FROM music "
    #     # cursor.execute(max_id_music)
    #     # max_id_music = cursor.fetchone()
    #     # max_id_music = max_id_music + 1
    #     b = (
    #         "INSERT INTO music (MusicName, Autor_id, Albom, Date, href) VALUES (%s, %s, %s, %s, %s)",
    #         (song_name, max_id_autor, albom, date_of_song, href_of_song))
    #     cursor.execute(b)
    #     # Сохранение изменений в БД
    #     connection.commit()
    #
    #     # Добавление элемента в список песен
    #     string_item = f"{song_name} - {autor_song}"
    #     item = QtWidgets.QListWidgetItem()
    #     font = QtGui.QFont()
    #     font.setPointSize(15)
    #     item.setFont(font)
    #     item.setCheckState(QtCore.Qt.Checked)
    #     self.Add_songs_list.addItem(string_item)
    #
    #     __sortingEnabled = self.Add_songs_list.isSortingEnabled()
    #     self.Add_songs_list.setSortingEnabled(False)
    #     item = self.Add_songs_list.item(0)
    #     self.Add_songs_list.setSortingEnabled(__sortingEnabled)
    #
    #     self.to_created_playlist()
    #
    #     cursor.close()
